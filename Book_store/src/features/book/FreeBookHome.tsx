...
export default function FreeBookHome() {
  ...
  const { books, isLoading,skeletonCount } = useFetch("https://api.itbook.store/1.0/new")

  console.log(skeletonCount,"skeletn")
            ...
            <div className='p-3 text-sm space-y-2 flex-1'>
              {item.subtitle.trim() ? 
                <p className='font-medium'>
                  <span className='text-gray-700'>Subtitle:</span> {item.subtitle}
                </p>
                :
                <p className='font-medium'>
                  <span className='text-gray-700'>Subtitle:</span> {item.title}
                </p>  
            }
              <p>
